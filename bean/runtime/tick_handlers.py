...
def build_default_handlers(
    ...
    *,
    ...
    if coherence_engine is not None:
        registry.register("coherence_window", lambda tick, session_uuid, ctx: coherence_engine.run(session_uuid, trigger="tick"), interval=coherence_interval)
    if consolidation_engine is not None:
        registry.register("consolidation", lambda tick, session_uuid, ctx: consolidation_engine.run(session_uuid, trigger="tick"), interval=consolidation_interval)
    ...
    def scheduled_reflection(tick: int, session_uuid: str, ctx: dict):
        ...
        if consolidation_engine is not None:
            consolidation_engine.run(session_uuid, trigger="post_reflection")
